---
title: Flutter Giriş
description: Bloc ile Flutter giriş akışının nasıl oluşturulacağına dair kapsamlı bir rehber.
sidebar:
  order: 4
---
...
- [Equatable](/tr/faqs#when-to-use-equatable) ile gereksiz yeniden
...
`package:authentication_repository`, herhangi bir harici bağımlılığı olmayan
saf bir Dart paketi olacaktır.

:::

Sırada, `packages/authentication_repository/lib/src/authentication_repository.dart`
...
İlk tanımlayacağımız şey,
`packages/user_repository/lib/src/models/user.dart` dosyasındaki kullanıcı
modelidir:
...
pratikte `firstName`, `lastName`, `avatarUrl` gibi ek özelliklerimiz
olabilir...
...
`packages/user_repository/lib/src/user_repository.dart` içinde
uygulayabiliriz.
...
`user_repository` paketi neredeyse bitti -- yapmamız gereken tek şey, dışa
açık aktarımları tanımlayan `user_repository.dart` dosyasını
...
`authentication_repository` ve `user_repository` paketleri tamamlandığına
göre, Flutter uygulamasına odaklanabiliriz.
...
yeni `AuthenticationState` örnekleri yaymak için işlenip kullanılacaktır.
...
sunum katmanı tarafından tüketilecektir.
...
`AuthenticationEvent` ve `AuthenticationState` uygulamalarını gördüğümüze
göre, şimdi `AuthenticationBloc`'a bir göz atalım.

### authentication_bloc.dart

`AuthenticationBloc`, kullanıcının bir giriş sayfasında mı yoksa bir ana
sayfada mı başlatılacağı gibi şeyleri belirlemek için kullanılan uygulamanın
kimlik doğrulama durumunu yönetir.
...
bir bağımlılığa sahiptir ve başlangıç durumunu
`AuthenticationState.unknown()` olarak tanımlar.
...
değişikliklerini loglamak için harikadır.;
...
oluşturmaktan/sağlamaktan sorumludur. Bu ayrıştırma, daha sonra hem `App`
hem de `AppView` widget'larını kolayca test etmemizi sağlayacaktır.
...
`create`'i çağırmaz. `AuthenticationBloc`'un her zaman
`AuthenticationStatus` stream'ine hemen abone olması gerektiğinden
...
Splash özelliği, kullanıcının kimliğinin doğrulanıp doğrulanmadığını
belirlerken uygulama başlatılır başlatılmaz render edilecek basit bir görünüm
içerecektir.
...
ona kolayca yönlendirmeyi sağlayan statik bir `Route` sunar;
...
oluşturmak amacıyla
[`package:formz`](https://pub.dev/packages/formz) kullanıyoruz.
...
doğrulama yapıyoruz, ancak pratikte özel karakter kullanımı, uzunluk vb.
...
Yine, parolanın boş olmadığından emin olmak için basit bir kontrol
yapıyoruz.

#### Models Barrel

Daha önce olduğu gibi, `Username` ve `Password` modellerini tek bir içe
aktarma ile içe aktarmayı kolaylaştıran bir `models.dart` barrel'ı vardır.
...
kullanılır ve status da [package:formz](https://pub.dev/packages/formz)
...
`AuthenticationRepository`'e bir bağımlılığa sahiptir. Bloc'un başlangıç
durumu `pure`'dur, yani ne girdilere ne de forma dokunulmuş veya etkileşime
geçilmiştir.
...
`LoginPage`, `Route`'u sunmaktan ve ayrıca `LoginBloc`'u oluşturup
`LoginForm`'a sağlamaktan sorumludur.
...
`LoginForm`, `LoginBloc`'u kullanıcı olayları hakkında bilgilendirmeyi
yönetir ve ayrıca `BlocBuilder` ve `BlocListener` kullanarak durum
değişikliklerine yanıt verir.
...
`BlocListener`, giriş gönderimi başarısız olursa bir `SnackBar` göstermek
için kullanılır. Ek olarak, `context.select`, gereksiz yeniden oluşturmaları
önleyerek her widget için `LoginState`'in belirli bölümlerine verimli bir
şekilde erişmek için kullanılır. `onChanged` geri çağrısı, kullanıcı
...
`authenticated`'a değişecek ve kullanıcı, kullanıcının `id`'sini ve çıkış
yapmak için bir düğmeyi görüntülediğimiz `HomePage`'e yönlendirilecektir.
...
aracılığıyla erişebilir ve bunu bir `Text` widget'ı aracılığıyla
görüntüler. Ek olarak, çıkış düğmesine dokunulduğunda, `AuthenticationBloc`'a
bir `AuthenticationLogoutPressed` olayı eklenir.
...
`context.select((AuthenticationBloc bloc) => bloc.state.user.id)`,
kullanıcı id'si değişirse güncellemeleri tetikleyecektir.
